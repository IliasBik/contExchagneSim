...
def table_config(run: D.RunData) -> str:
    ...
    comments = {
        ...
        "arb_vol": "дисперсия базиса арбитражёров: venues (по биржам) или ce basis (EWMA)",
        "marks": "цены оценки PnL: fair (закрытие позиции) или ce",
...
def values(run: D.RunData, s: dict) -> dict:
    ...
    return {
        ...
        "exec_t2": signed(kinds["T2"]["exec"], 3),
...
Прибыль считается по ценам оценки \(P_i\): \(\mathrm{PnL} = \sum_i q_i P_i\)
в единицах \(X_1\). По умолчанию это цены закрытия позиции
(\texttt{marks = "fair"}): \(P_{X_1} = P_{X_2} = 1\), а \(P_{Y_v}\) --- мид
лимитной биржи \(v\). Цены самой CE для оценки не годятся: их задают те же
агенты, и при большом инвентаре они отрываются от справедливых. Стартовый
капитал \(C_0\) на баланс не кладётся, он живёт только в формулах доли и
риска, поэтому прибыль равна ровно стоимости реального портфеля.

Внутри тика количества меняются дважды --- в клиринге CE и в хедже, --- и
оба раза по известным ценам. Отсюда точное разложение изменения прибыли:
\[
  \Delta \mathrm{PnL}_t
  = \underbrace{\sum_i q_{i,t-1}\bigl(P_{i,t} - P_{i,t-1}\bigr)}
    _{\text{переоценка позиции}}
  + \underbrace{\sum_i \Delta q^{\text{сделки}}_{i}\,P_{i,t}}
    _{\text{исполнение на CE}}
  + \underbrace{\sum_i \Delta q^{\text{хедж}}_{i}\,P_{i,t}}
    _{\text{результат хеджа}} .
\]
Средний член --- выигрыш на сделке CE относительно цен оценки. Портфель
заявки самофинансируем (\(\sum_i w_i = 0\)), поэтому по ценам самой CE сделка
не создаёт и не уничтожает стоимость, а по справедливым ценам она даёт ровно
захваченную долю расхождения цен CE с паритетом. Третий член --- то, что
агент выиграл или потерял, скидывая инвентарь по встречной стороне книги.
Невязка этого тождества за весь прогон не превысила \($residual\) X1 --- то
есть разложение на рис.~\ref{fig:agents_structure} не приближённое, а точное.
"""
...
\toprule
семейство & выжило & PnL, X1 & переоценка & исполн. CE & хедж & оборот, X1
& средняя \(|q|\) \\
\midrule
$table_kinds
\bottomrule
...
лимитные биржи. Его заработок --- исполнение на CE: он покупает базис ниже
паритета и продаёт выше, когда трансляторы возвращают цены к нему;
переоценка набранной позиции по мидам бирж --- лишь шум базиса между
площадками. Шейдинг у арбитражёров по умолчанию выключен (их портфель
самохеджирован), так что позицию сдерживает только встречный поток
трансляторов.

У транслятора картина обратная. Он продаёт свой портфель на CE с премией к
паритету, но вынужден периодически скидывать инвентарь по встречной стороне
книги, теряя полуспред; на тонкой площадке спред шире ($spread2 против
$spread1), и это прямо переносится в результат. Показательно семейство
\(T_2\): исполнение на CE дало $exec_t2 X1, переоценка позиции ---
$reval_t2 X1, а хедж --- $hedge_t2 X1; итоговый $pnl_t2 X1 --- разность
