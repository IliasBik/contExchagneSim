...
class SimConfig:
    ...
    # По умолчанию — точка плато оптимума из ветки optimization (справедливые
    # марки, идеальные арбитражёры; mean_field.md §13, overnight/README.md).
    h_m1_values: tuple = (0.0164, )
    h_r1_values: tuple = (0.228, )
    h_m2_values: tuple = (0.0061, )
    h_r2_values: tuple = (1.64, )
    ...
    # книги был на два порядка больше типичного хеджа агентов, и глубина
    # стакана никогда не ограничивала их оборот (ветка optimization).
    venue1: ExchangeConfig = field(default_factory=lambda: ExchangeConfig(
        name="1", arrival_rate=10.0, order_size=0.05, order_ttl=5,
        price_std=0.5, ewma_half_life=10.0))
    venue2: ExchangeConfig = field(default_factory=lambda: ExchangeConfig(
        name="2", arrival_rate=3.0, order_size=0.05, order_ttl=5,
        price_std=0.5, ewma_half_life=10.0))
